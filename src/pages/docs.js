...
const DocsCard = ( { doc } ) => {
  const image = getImage( doc.featuredImage );
  return (
    <div className = 'w-full px-0 lg:px-8 my-4 w-full sm:px-4 sm:w-1/2 xl:w-1/3'>
      <div className = 'bg-white rounded-lg overflow-hidden shadow-md relative'>
        ...
        <div className = 'flex flex-col p-4 h-auto md:h-40 lg:h-48'>
          <Link to = { `/docs/${ doc.id }/` } className = 'flex-initial block text-gray-800 hover:text-blue-600 font-semibold mb-2 text-lg md:text-base lg:text-lg transition-colors duration-500 ease-in-out'>
            { doc.title }
          </Link>
          <div className = 'flex-auto text-gray-700 text-sm leading-relaxed block md:text-xs lg:text-sm'>
            { doc.description }
          </div>
